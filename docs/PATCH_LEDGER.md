...
| P4a | `packages/web/vite.config.ts` | `manualChunks` takes the segment after the LAST `node_modules/` | bun hoists under `node_modules/.bun/<pkg>@<ver>/node_modules/<pkg>/`; the first-segment parse classified every dep as ".bun", collapsing the vendor tree into ONE eager ~3.8MB gz chunk. Caught by the new bundle gate; initial payload 4010.7→592.1 KB gz |
