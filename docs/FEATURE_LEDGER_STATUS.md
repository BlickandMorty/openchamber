...
| Chat + streaming + tool UIs + diffs | **GREEN (both engines, e2e)** | opencode: keystroke turn through the vendored UI streamed "ready" (Big Pickle, screenshot). **goose: FULL TURN PROVEN 2026-07-04** — the exact adapter sequence (start -> adopt the user's active_provider `cursor-agent`/`auto` via /config/read + /agent/update_provider -> /reply) streams a real MessageEvent + Finish through the app's live goosed proxy. Fixed the missing provider-selection step (`326d8ad8`). No owner key needed — they already had goose configured |
